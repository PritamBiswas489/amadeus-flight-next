...
export default function LeftSearchPanel() {
   ...
   const {locations, carriers} = useSelector((state)=>state['offerData'].dictionaries); 
                                ...
                                <div className={style.leftFilterInner}>
                                  ...
                                  {enableFilter.enableFilterStops && <FilterStops/>}  
                                    ...
                                    <div className={`leftFilterInnerBox`}>
                                        ...
                                        {enableFilter.enableFilterAccordianOnwardStops && !nonStop && <FilterAccordianOnwardStops/> }
                                            {enableFilter.enableFFilterAccordianReturnStops && !nonStop && flightType ==='RETURN' && <FilterAccordianReturnStops/> }
                                            
                                            {enableFilter.enableFilterAccordianDepartureTiming &&<FilterAccordianDepartureTiming/>}
                                            {enableFilter.enableFilterAccordianReturnTiming && flightType ==='RETURN' && <FilterAccordianReturnTiming/>}


                                            {enableFilter.enableFilterAccordianPrice && <FilterAccordianPrice/>}    
                                            {enableFilter.enableFilterAccordianDurationDeparture && <FilterAccordianDurationDeparture/> }
                                            {enableFilter.enableFilterAccordianDurationReturn && flightType ==='RETURN' && <FilterAccordianDurationReturn/>}
                                            {enableFilter.enableFilterAccordianTimeOfFlight && <FilterAccordianTimeOfFlight/>} 
                                            {enableFilter.enableFilterAccordianAirline && <FilterAccordianAirline carriers={carriers}/>} 
                                            {enableFilter.enableFilterAccordianAirports && <FilterAccordianAirports locations={locations}/>}
                                           
                                            
                                            


                                            {enableFilter.enableFilterAccordianFreeAssistant && <FilterAccordianFreeAssistant/>}  
                                            {enableFilter.enableFilterAccordianDuration && <FilterAccordianDuration/>} 
                                            {enableFilter.enableFilterAccordianFlightQuality && <FilterAccordianFlightQuality/>} 
